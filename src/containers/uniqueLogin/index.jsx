...
import { fetchRetainUsers, fetchEmpDetails } from "store/retain/action";
import { retainUsersTableHeader, initialTableConfig } from "constants/retain";
import moment from "moment";
import { getTimezoneOffset } from "utils/functions";
import { retainUsersListSelector } from "store/retain/selector";
...
class Retain extends Component {
	...
	handleActions = (actionType, row) => {
		...
		if (actionType === "viewAction") {
			this.props.history.push(
				...
				)}&endDate=${moment(endDate).format("ddd MMM DD YYYY HH:mm:ss")}&name=${
					row.name
	...
	render() {
		const { tableConfig, showInfo, empDetails } = this.state;

		const { retainUsersList, authInfo } = this.props;
